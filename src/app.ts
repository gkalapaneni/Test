...
import { Database, getDatabaseConfig } from './config/database';
...
export class TodoApplication {
  ...
  constructor() {
    this.app = express();
    this.apiSpec = path.join(__dirname, '../openapi.json');
  ...
  private loadSwaggerDocument(): void {
    try {
      this.swaggerDocument = require(this.apiSpec);
    } catch (error) {
    ...
    return new Promise((resolve, reject) => {
      try {
        this.server = this.app.listen(port, () => {
          ...
          console.log(`OpenAPI spec: ${this.apiSpec}`);
          resolve(this.server!);
        });
